...
namespace ResponseHaandle {
	...
	void initPathFromLocation() {
		...
		_filePath = Utils::getFilePath(RequestData::uri);
		if (_filePath.empty())
		{
			throw InternalServerError_500;
		}
	...
	namespace Utils {
		...
		std::string getFilePath(const std::string& uri) {
			...
			if (!_location->getCgiPath().empty()) {
				std::string cgiPath = normalizePath(_location->getCgiPath());
			...
			// 기본 경로 설정
			std::string baseFilePath = !_location->getRootPath().empty() ? normalizePath(_location->getRootPath()) : _server.getRootPath();
